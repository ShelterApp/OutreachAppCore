import { Controller, Get, Post, Body, Patch, Param, Delete, UseInterceptors, UseGuards, Request, NotFoundException, Put, Res, Req } from '@nestjs/common';
import { UpdateProfileDto } from './dto/update-profile.dto';
import { UsersService } from 'src/users/users.service';
import { SanitizeMongooseModelInterceptor } from 'nestjs-mongoose-exclude';
import { JwtAuthGuard } from 'src/auth/jwt-auth.guard';
import { ApiBearerAuth, ApiNotFoundResponse, ApiOkResponse, ApiTags, ApiUnprocessableEntityResponse, ApiBadRequestResponse, ApiOperation } from '@nestjs/swagger';
...
@ApiBearerAuth()
@ApiUnprocessableEntityResponse({status: 422, description: 'cannot process entity'})
export class ProfileController {
  ...
  @ApiNotFoundResponse({status: 404, description: 'cannot found user'})
  @ApiUnprocessableEntityResponse({status: 422, description: 'unprocess entity'})
  @ApiBadRequestResponse({status: 400, description: 'wrong password'})
  async changePassword(@Request() req, @Body() changePasswordDto: ChangePasswordDto, @Res() res: Response) {
    const id = req.user.id;
