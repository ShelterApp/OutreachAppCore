...
export class SuppliesController {
  ...
  @ApiOperation({ summary: 'Get list supplies (Role: Admin, OrgLead)' })
  ...
  async find(@Query() { skip, limit }: PaginationParams, @Query() searchParams: SearchParams) {
    const [items, total] = await this.suppliesService.findAll(searchParams, skip, limit);
    return {
      items,
      total
    };
  ...
  @ApiOkResponse({status: 200, description: 'supplies object'})
  async update(@Param() { id }: ParamsWithId, @Body() updateSupplyDto: UpdateSupplyDto) {
